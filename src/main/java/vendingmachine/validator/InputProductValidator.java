...
public class InputProductValidator {
    ...
    private static int validatePrice(String price) {
        ...
        validateNumberBase(parsedPrice);
        return parsedPrice;
    }
    ...
    private static int validateQuantity(String quantity) {
        ...
        try {
            ...
            throw new IllegalArgumentException("[ERROR] 수량은 정수로 주어져야 합니다.");
        }
    ...
    private static void validateMinimum(int price) {
        if (price < 100) {
            throw new IllegalArgumentException("[ERROR] 상품 가격은 최소 100원 이상이어야 합니다.");
        }
    }

    private static void validateNegativeOrZero(int price) {
        if (price <= 0) {
            throw new IllegalArgumentException(("[ERROR] 0 또는 음수는 입력될 수 없습니다."));
        }
    }

    private static void validateNumberBase(int price) {
        if (price % 10 != 0) {
            throw new IllegalArgumentException("[ERROR] 상품 가격은 10원 단위여아 합니다.");
        }
    }

    private static void validateSpace(String input) {
        if (input.contains(" ")) {
            throw new IllegalArgumentException("[ERROR] 상품명에 공백이 존재합니다.");
        }
    }
}
